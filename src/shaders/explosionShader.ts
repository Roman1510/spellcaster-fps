  ...
  return new ShaderMaterial({
    ...
    vertexShader: `
      ...
      void main() {
        ...
        displacement *= (1.0 - uAge * 0.5);
        
        // Expand outward with some noise
        pos += normal * (uExpansion + displacement);
        
        // Add some fast flickering movement
        pos += normal * sin(uTime * 20.0 + position.y * 10.0) * 0.05 * (1.0 - uAge);
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        if (uAge < 0.2) {
          explosionShape = 1.0 - smoothstep(0.0, 0.3, length(vUv - 0.5));
        } else {
          // As it ages, it becomes more chaotic and hollow
          float hollowness = smoothstep(0.2, 1.0, uAge);
          float innerRadius = 0.2 * hollowness;
          float dist = length(vUv - 0.5);
          explosionShape = smoothstep(innerRadius, 0.5, dist) * (1.0 - smoothstep(0.3, 0.5, dist));
          explosionShape += combinedNoise * 0.4 * (1.0 - uAge);
        }
        
        // Add fresnel for edge glow
        explosionShape += fresnel * 0.3 * (1.0 - uAge);
        ...
        float colorMix = combinedNoise * 0.5 + 0.5 + uAge;
        ...
        fireColor = mix(fireColor, smokeColor, smoothstep(0.5, 1.5, uAge));
        
        // Hot spots and bright areas
        float hotSpots = pow(max(0.0, noise1), 2.0) * (1.0 - smoothstep(0.0, 0.4, uAge));
        fireColor += vec3(0.5, 0.2, 0.0) * hotSpots;
        
        // Calculate opacity - fade out over time
        float alpha = explosionShape * uOpacity;
        alpha *= (1.0 - smoothstep(0.0, 1.0, uAge * 0.7));
        
        // Add flicker for realism
        float flicker = 0.8 + 0.2 * sin(uTime * 30.0 + combinedNoise * 20.0);
        flicker = mix(1.0, flicker, 1.0 - uAge); // Less flicker as it ages
